...
export class LdspocEventListComponent implements OnInit, OnDestroy {
  ...
  getStatusClass(status: string): string {
    switch (status?.toLowerCase()) {
      ...
      case 'in progress':
        return 'status-in-progress';
      case 'completed':
        return 'status-completed';  // *** Uses bluish-green gradient ***
      case 'cancelled':
      case 'rejected':
        return 'status-rejected';
      case 'approved':
  ...
  onCompleteEvent(event: EventViewDetails): void {
    ...
    if (!confirm(confirmMessage)) {
      return; // User clicked Cancel, don't open modal
    }

    // User clicked OK, proceed to open the completion modal
    this.selectedEventForApproval = event;
    this.isApprovalAction = true;
    this.approvalNotes = '';
    this.submissionError = '';
  ...
  onCancelEvent(event: EventViewDetails): void {
    ...
    const confirmMessage =
      ...
      '⚠️ All linked requests will be UNLINKED from this event\n' +
      '⚠️ Request statuses will be changed back to "Approved"\n' +
    ...
    // User clicked OK, proceed to open the cancellation modal
    this.selectedEventForApproval = event;
    this.isApprovalAction = false;
    this.approvalNotes = '';
    this.submissionError = '';
  ...
  // Submit Event Completion/Cancellation - Backend handles request status updates
  submitEventApproval(): void {
    // Validate notes
    if (!this.approvalNotes || this.approvalNotes.trim().length < 10) {
      this.showNotesError = true;
    ...
    this.eventService.updateEvent(this.selectedEventForApproval.eventId, eventData)
      ...
      .subscribe(response => {
        if (response) {
          ...
          const message = this.isApprovalAction
            ? `Event "${this.selectedEventForApproval?.eventName}" has been COMPLETED.\n\n` +
            `✓ All linked requests have been marked as "Completed"\n` +
            `✓ Completed by: ${this.currentUser!.cdsId}`
            : `⚠️ Event "${this.selectedEventForApproval?.eventName}" has been CANCELLED.\n\n` +
            `✓ All linked requests have been UNLINKED\n` +
            `✓ Request statuses changed back to "Approved"\n` +
            `✓ Cancelled by: ${this.currentUser!.cdsId}`;
  ...
  // Helper method to check if Complete button should be shown ***
  // Show Complete button for all statuses EXCEPT Deleted and Completed
  shouldShowCompleteButton(event: EventViewDetails): boolean {
    const status = event.status.toLowerCase();
    // Hide if Deleted or Completed
    // Show (but disabled) if Cancelled
    return status !== 'deleted' && status !== 'completed';
  }

  // Helper method to check if Cancel button should be shown ***
  // Show Cancel button for all statuses EXCEPT Deleted and Cancelled
  shouldShowCancelButton(event: EventViewDetails): boolean {
    const status = event.status.toLowerCase();
    // Hide if Deleted or Cancelled
    // Show (but disabled) if Completed
    return status !== 'deleted' && status !== 'cancelled';
  }

  // *** REQUIREMENT 2: Helper method to check if Complete button should be disabled ***
  ...
  // *** REQUIREMENT 2: Helper method to check if Cancel button should be disabled ***
